...
==================================================
Filename: figures/fig4_structure_improvement.tex
==================================================
% \begin{forest}
%     for tree={
%         grow=east,
...
%     [Improvement\\Strategy, fill=green!20
%         [Design Strategy of \\Evaluation Prompts \\(Sec.~\ref{subsec:design_prompts}), text width=8em
%             [Optimizing LLMs’ \\Understanding of \\Evaluation Tasks, text width=6em
%                 [\emph{Few-shot prompting:} FActScore~\cite{min2023factscore} / SALAD-Bench~\cite{salad_bench} / GPTScore~\cite{fu2024gptscore}]
%                 [\emph{Evaluation steps decomposition:} G-Eval~\cite{geval} / DHP~\cite{dhp} / SocREval~\cite{he2024socreval} / BSM~\cite{bsm}]
%                 [\emph{Evaluation criteria decomposition:} HD-Eval~\cite{hd-eval} / Hu and Gao et al.~\cite{hu-etal-2024-llm}]
%                 [\emph{Shuffling contents:} Wang et al.~\cite{wang-etal-2024-large-language-models-fair} / Auto-J~\cite{auto-j} / JudgeLM~\cite{zhu2023judgelm} / PandaLM~\cite{wang2023pandalm}]
%                 [\emph{Conversion of evaluation tasks:} Liu et al.~\cite{liu2024aligning}]
%             ]
%             [Optimizing LLMs’\\ Output Forms, text width=6em
%                 [\emph{Constraining outputs in structured formats:} G-Eval~\cite{geval} / DHP~\cite{dhp} / LLM-EVAL~\cite{llmeval}]
%                 [\emph{Providing evaluations with explanations:} CLAIR~\cite{chan2023clair} / FLEUR~\cite{lee-etal-2024-fleur}]
%             ]
%         ]
%         [Improvement Strategy \\of LLMs’ Abilities \\(Sec.~\ref{subsec:improve_models}), text width=8em)
%             [Fine-tuning via Meta \\Evaluation Datasets, text width=7em
%                 [\emph{Evaluation template:} PandaLM~\cite{wang2023pandalm} / SALAD-Bench~\cite{salad_bench}]
%                 [\emph{Deep transformation:} OffsetBias~\cite{offsetbias} / JudgeLM~\cite{zhu2023judgelm} / CritiqueLLM~\cite{ke2024critiquellm} / Yu et al.~\cite{yu2025improve}]
%             ]
%             [Iterative Optimization \\Based on Feedbacks, text width=7em
%                 [INSTRUCTSCORE~\cite{xu2023instructscore} / JADE~\cite{zhang2023jade} / Think-J~\cite{huang2025think}]
%             ]
%         ]
%         [Optimization Strategy \\of Final Results \\(Sec.~\ref{subsec:optimization_results}), text width=8em)
%             [Integration of multiple \\Evaluation Results, text width=7.5em
%                 [\emph{Summarize by multiple rounds:} Sottana et al.~\cite{sottana2023evaluation} / PsychoBench~\cite{psycho_bench} / Auto-J~\cite{auto-j}]
%                 [\emph{Vote by multiple LLMs:} CPAD~\cite{cpad} / Bai et al.~\cite{lm-as-an-examiner}]
%                 [\emph{Hierarchical evaluation framework:} Jung et al.~\cite{jung2024trust} / Zhang et al.~\cite{zhang2025crowd}]
%             ]
%             [Direct Optimization of \\LLMs’ Outputs, text width=7.5em
%                 [\emph{Score smoothing:} FLEUR~\cite{lee-etal-2024-fleur} / G-Eval~\cite{geval} / DHP~\cite{dhp}]
%                 [\emph{Self validation:} TrueTeacher~\cite{gekhman2023trueteacher}]
%             ]
%         ]
%     ]
% \end{forest}
...
\begin{forest}
    ...
    where level=3{font=\footnotesize,yshift=0.26pt,fill=yellow!20}{},
    [Improvement\\Strategy, fill=green!20
        [Prompt Design Strategy\\(Sec.~\ref{subsec:design_prompts}), text width=9em
            [Improving LLMs' Task\\Understanding, text width=9em
                [\emph{Few-shot prompting:} FActScore~\cite{min2023factscore} / SALAD-Bench~\cite{salad_bench} / GPTScore~\cite{fu2024gptscore}]
                [\emph{Evaluation steps decomposition:} G-Eval~\cite{geval} / DHP~\cite{dhp} / SocREval~\cite{he2024socreval} / BSM~\cite{bsm}]
                [\emph{Evaluation criteria decomposition:} HD-Eval~\cite{hd-eval} / Hu and Gao et al.~\cite{hu-etal-2024-llm}]
                [\emph{Shuffling contents:} Wang et al.~\cite{wang-etal-2024-large-language-models-fair} / Auto-J~\cite{auto-j} / JudgeLM~\cite{zhu2023judgelm} / PandaLM~\cite{wang2023pandalm}]
            ...
            [Standardizing LLMs'\\ Output Format, text width=9em
                [\emph{Constraining outputs in structured formats:} G-Eval~\cite{geval} / DHP~\cite{dhp} / LLM-EVAL~\cite{llmeval}]
                [\emph{Providing evaluations with explanations:} CLAIR~\cite{chan2023clair} / FLEUR~\cite{lee-etal-2024-fleur}]
            ]
        ]
        [Capability Enhancement\\ Strategy\\(Sec.~\ref{subsec:improve_models}), text width=9em)
            [Specialized Fine-tuning, text width=9em
                [\emph{Evaluation template:} PandaLM~\cite{wang2023pandalm} / SALAD-Bench~\cite{salad_bench}]
                [\emph{Deep transformation:} OffsetBias~\cite{offsetbias} / JudgeLM~\cite{zhu2023judgelm} / CritiqueLLM~\cite{ke2024critiquellm} / Yu et al.~\cite{yu2025improve}]
            ]
            [Feedback-Driven Iterative\\Refinement, text width=9em
                [INSTRUCTSCORE~\cite{xu2023instructscore} / JADE~\cite{zhang2023jade} / Think-J~\cite{huang2025think}]
            ]
        ]
        [Final Output\\ Optimization Strategy \\(Sec.~\ref{subsec:optimization_results}), text width=9em)
            [Integrating Multi-Source\\ Evaluation Results, text width=9em
                [\emph{Summarize by multiple rounds:} Sottana et al.~\cite{sottana2023evaluation} / PsychoBench~\cite{psycho_bench} / Auto-J~\cite{auto-j}]
                [\emph{Vote by multiple LLMs:} CPAD~\cite{cpad} / Bai et al.~\cite{lm-as-an-examiner}]
                [\emph{Hierarchical evaluation framework:} Jung et al.~\cite{jung2024trust} / Zhang et al.~\cite{zhang2025crowd}]
            ]
            [Direct Output Optimization, text width=9em
                [\emph{Score smoothing:} FLEUR~\cite{lee-etal-2024-fleur} / G-Eval~\cite{geval} / DHP~\cite{dhp}]
                [\emph{Self validation:} TrueTeacher~\cite{gekhman2023trueteacher}]
            ]
        ]
...
==================================================
Filename: figures/chap3_structure_improve.tex
==================================================
\begin{forest}
    for tree={
        grow=east,
    ...
    where level=1{text width=5em,fill=green!20}{}, 
    where level=2{text width=10em,fill=blue!10}{},
    where level=3{text width=9em,fill=pink!30}{},
    where level=4{yshift=0.26pt,fill=yellow!20}{},
    [Reliable\\LLM-as-a-Judge, fill=orange!20
        [Improvement\\(Sec.~\ref{sec:improvement}),
            [Prompt Design Strategy\\(Sec.~\ref{subsec:design_prompts})
            ...
            [Capability Enhancement\\ Strategy\\(Sec.~\ref{subsec:improve_models})
                [Specialized Fine-tuning
                    [\emph{Evaluation template:} PandaLM~\cite{wang2023pandalm} / SALAD-Bench~\cite{salad_bench}]
                    [\emph{Deep transformation:} OffsetBias~\cite{offsetbias} / JudgeLM~\cite{zhu2023judgelm} /\\ CritiqueLLM~\cite{ke2024critiquellm} / Yu et al.~\cite{yu2025improve}]
                ]
                [Feedback-Driven Iterative\\Refinement, text width=10em
                    [INSTRUCTSCORE~\cite{xu2023instructscore} / JADE~\cite{zhang2023jade} / Think-J~\cite{huang2025think}]
                ]
            ]
            [Final Output\\ Optimization Strategy \\(Sec.~\ref{subsec:optimization_results})
                [Integrating Multi-Source\\ Evaluation Results
                    [\emph{Summarize by multiple rounds:} Sottana et al.~\cite{sottana2023evaluation} /\\ PsychoBench~\cite{psycho_bench} / Auto-J~\cite{auto-j}]
                    [\emph{Vote by multiple LLMs:} CPAD~\cite{cpad} / Bai et al.~\cite{lm-as-an-examiner} / EvalMORAAL~\cite{mohammadi2025evalmoraal}]
                    [\emph{Hierarchical evaluation framework:} Jung et al.~\cite{jung2024trust} / Zhang et al.~\cite{zhang2025crowd}]
                ]
                [Direct Output Optimization, text width=11em
                    [\emph{Score smoothing:} FLEUR~\cite{lee-etal-2024-fleur} / G-Eval~\cite{geval} / DHP~\cite{dhp} / TrustJudge~\cite{wang2025trustjudge}]
                    [\emph{Self validation:} TrueTeacher~\cite{gekhman2023trueteacher}]
                ]
            ]
        ]
        [Evaluation\\(Sec.~\ref{sec:Evaluation})
            [Evaluation of Agreement \\with Human Judgments \\(Sec.~\ref{Evaluation:base metric})
                [Agreement~\cite{thakur2024judging} Cohen's Kappa~\cite{thakur2024judging} \\Spearman's correlation ~\cite{liu2024aligning,lm-as-an-examiner}, text width=15em
                ]
            ]
            [Evaluation of Bias \\(Sec.~\ref{Evaluation:Bias})
                [Task-Agnostic Biases \\(Sec.~\ref{Evaluation:Task-Agnostic Biases})
                    [Diversity Bias~\cite{ye2024justice} Cultural Bias \\Self-Enhancement Bias~\cite{nips_llm_as_a_judge,ye2024justice}]
                ]
                [Judgment-Specific Biases \\(Sec.~\ref{Evaluation:Judgment-Specific Biases}), text width=10em
                    [Position Bias\cite{wang-etal-2024-large-language-models-fair, thakur2024judging, shi2024judging, ye2024justice} \\Compassion-fade bias~\citep{koo2023benchmarking, ye2024justice}
                    \\Style Bias~\cite{chen2024humans, ye2024justice,li2023examining}
                    \\Length Bias~\cite{offsetbias, huang2024_finetuned_judge, nips_llm_as_a_judge, ye2024justice}
                    \\Concreteness Bias~\cite{offsetbias,chen2024humans,ye2024justice}]
                ]
            ]
            [Evaluation of Adversarial \\Robustness (Sec.~\ref{Evaluation:Robustness})
                [Adversarial Phrases Attack~\cite{raina2024llm}, text width=13em
                ]
                [Null Model Attack~\cite{zheng2024cheating}
                ]
                [Majority Opinions Attack~\cite{ye2024justice,koo2023benchmarking}, text width=13em
                ]
                [Meaningless Statement Robustness~\cite{ye2024justice,koo2023benchmarking}, text width=17em
                ]
            ]
        ]
    ]
...
==================================================
Filename: figures/fig_meta_evaluation.tex
==================================================
\begin{forest}
    for tree={
        grow=east,
    ...
    where level=2{text width=5em,font=\footnotesize,fill=pink!30}{},
    where level=3{font=\footnotesize,yshift=0.26pt,fill=yellow!20}{},
    [Meta\\-Evaluation, fill=green!20
        [Evaluation of Agreement \\with Human Judgments \\(Sec.~\ref{Evaluation:base metric}), text width=10em
            [Agreement~\cite{thakur2024judging} Cohen's Kappa~\cite{thakur2024judging} \\Spearman's correlation ~\cite{liu2024aligning,lm-as-an-examiner}, text width=15em
            ]
        ]
        [Evaluation of Bias \\(Sec.~\ref{Evaluation:Bias}), text width=10em
            [Task-Agnostic Biases \\(Sec.~\ref{Evaluation:Task-Agnostic Biases}), text width=7em
                [Diversity Bias~\cite{ye2024justice} Cultural Bias \\Self-Enhancement Bias~\cite{nips_llm_as_a_judge,ye2024justice}]
            ]
            [Judgment-Specific Biases \\(Sec.~\ref{Evaluation:Judgment-Specific Biases}), text width=8em
                [Position Bias\cite{wang-etal-2024-large-language-models-fair, thakur2024judging, shi2024judging, ye2024justice} \\Compassion-fade bias~\citep{koo2023benchmarking, ye2024justice}
                \\Style Bias~\cite{chen2024humans, ye2024justice,li2023examining}
                \\Length Bias~\cite{offsetbias, huang2024_finetuned_judge, nips_llm_as_a_judge, ye2024justice}
                \\Concreteness Bias~\cite{offsetbias,chen2024humans,ye2024justice}]
        ...
        [Evaluation of Adversarial \\Robustness (Sec.~\ref{Evaluation:Robustness}), text width=10em
            [Adversarial Phrases Attack~\cite{raina2024llm}, text width=11em
            ]
            [Null Model Attack~\cite{zheng2024cheating}, text width=8em
            ]
            [Majority Opinions Attack~\cite{ye2024justice,koo2023benchmarking}, text width=13em
            ]
            [Meaningless Statement Robustness~\cite{ye2024justice,koo2023benchmarking}, text width=15em
            ]
        ]
    ]
\end{forest}
...
\input{tex/2_Formulation}
\input{tex/3_Improvement/3_Improvement}
\input{tex/4_Evaluation}
\input{tex/5_Experiments}
\input{tex/5_1_o1}
% \input{tex/0_towards_AGI}
\input{tex/6_Application}
\input{tex/7_Challenge}
\input{tex/8_Future_work}
\input{tex/9_Conclusion}
...
==================================================
Filename: tables/exp_result_models.tex
==================================================
\begin{table*}[t]
    \small
    \centering
    ...
    \begin{tabular}{lcccccccc}
        \toprule
        \multirow{4}{*}{LLMs} & Alignment & \multicolumn{7}{c}{Biases} \\
        & with & \multirow{2}{*}{Position} & \multirow{2}{*}{Length} & Concre- & Empty & Content & Nested & Familiar \\
        ...
        Mixtral-8$\times$7B-Instruct-v0.1 & 56.29 & 59.06 & 50.00 & 78.57 & 42.31 & 83.33 & 29.17 & 83.33 \\
        \midrule
    ...
    \end{tabular}
    \end{adjustbox}
    \caption{The meta-evaluation results for different LLMs. All the values are percentages.}
    \label{tb:exp_results_models}
    \vspace{-20pt}
\end{table*}


==================================================
Filename: tables/exp_result_strategies.tex
==================================================
\begin{table*}[t]
    \small
    \centering
    ...
    \begin{tabular}{lcccccccc}
        \toprule
        \multirow{4}{*}{Improvement Strategies} & Alignment & \multicolumn{7}{c}{Biases} \\
        & with & \multirow{2}{*}{Position} & \multirow{2}{*}{Length} & Concre- & Empty & Content & Nested & Familiar \\
        & Human & & & teness & Reference & Continuation & Instruction & Knowledge \\
        & (n=5106) & (n=2633) & (n=34) & (n=28) & (n=26) & (n=24) & (n=24) & (n=24) \\
        \midrule
        ...
        \,- \textit{w/} explanation & 52.47 & 48.97 & 35.29 & 60.71 & 38.46 & 91.67 & 41.67 & 50.00 \\
        \,- \textit{w/} self-validation & 54.86 & 69.31 & 23.53 & 60.71 & 23.08 & 91.67 & 41.67 & 50.00 \\
        \,- \textit{w/} multi rounds & & & & & & & & \\ 
        \qquad- majority@5 & 54.68 & 70
        11 & 26.47 & 67.86 & 23.08 & 95.83 & 54.17 & 50.00 \\
        \qquad- mean@5 & 54.72 & 69.58 & 11.76 & 57.14 & 26.92 & 87.50 & 50.00 & 50.00 \\
        \qquad- best-of-5 & 51.95 & 58.72 & 5.88 & 42.86 & 19.23 & 87.50 & 37.50 & 45.83 \\
        multi LLMs (\textit{set 1}) & 57.66 & 32.28 & 26.47 & 64.28 & 46.15 & 87.50 & 66.67 & 62.50 \\ 
        multi LLMs (\textit{set 2}) & 58.19 & 70.98 & 64.71 & 71.43 & 69.23 & 91.67 & 45.83 & 83.33 \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
    \caption{The results for different strategies based on GPT-3.5-turbo. All the values are percentages.}
    \label{tb:exp_results_strategies}
    \vspace{-20pt}
\end{table*}


==================================================
Filename: tables/fine-tuned_evaluator.tex
==================================================
\begin{table*}[h]
...
\textbf{Model} & \textbf{Foundation} & \textbf{Instruction}     & \textbf{Response}       & \textbf{Annotation}          & \textbf{Evaluation Scheme }               & \textbf{Testset}  \\
{JudgeLM\\
\citep{judgelm}} & Vicuna        & {Instruct Datasets \\(Alpaca-GPT4, \\Dolly-15K...)}         & {11 models
\\(Alpaca,Vicuna...)} & GPT-4                & Pairwise Grading                        & {GPT-4}   \\
{PandaLM \\
\citep{wang2023pandalm}}& LLaMA         & Alpaca 52K                                                  & {5 models\\(LLaMA, Bloom...)}          & GPT3.5                       & Pairwise Selection                      & {Human} \\
{Auto-J\\
\citep{auto-j}} & LLaMA2-chat    & {Preference Datasets
\\(Chatbot Arena,\\OpenAI WebGPT...)}        & Preference Datasets       & Human            & {Pairwise Selection\\Pointwise Grading} & {Human}  \\
{Prometheus\\
\citep{prometheus}}& LLaMA2-chat   & GPT-4 Generated        & GPT-4 Generated                       & GPT-4         & Pointwise Grading                       & {GPT-4}   
\end{tblr}}
\caption{Detailed statistics of the four Evaluator models fine-tuned from open-source model, with their training and test data also publicly released.}
...
==================================================
Filename: tables/exp_all_results.tex
==================================================
\begin{table*}[t]
    \small
    \centering
    \begin{minipage}{\linewidth}
        ...
        \end{tabular}
        \end{adjustbox}
        \caption*{(a) The meta-evaluation results for different LLMs. All the values are percentages.}\label{tb:exp_results_models}
    \end{minipage}
    ...
    \begin{minipage}{\linewidth}
        ...
        \begin{tabular}{lcccccccc}
            \toprule
             \multirow{2}{*}{LLMs} & \multicolumn{2}{c}{human=model1} & \multicolumn{2}{c}{human=model2} & \multicolumn{2}{c}{human=TIE} \\
             % & \multicolumn{2}{c}{(n=2071)} & \multicolumn{2}{c}{(n=2071)} & \multicolumn{2}{c}{(n=964)} \\
        ...
        \end{tabular}
        \end{adjustbox}
        \caption*{(b) The evaluation results of each human label in LLMEval$^2$. Only valid responses are counted when calculating the accuracy, while samples that couldn't receive responses due to triggering Azure OpenAI's content management policy are excluded. So there are some differences in the total values of different models.}\label{tb:exp_results_llmeval2_detail}
    \end{minipage}
    ...
    \begin{minipage}{\linewidth}
        ...
        \end{tabular}
        \end{adjustbox}
        \caption*{(c) The meta-evaluation results for different strategies based on GPT-3.5-turbo. All the values are percentages.}\label{tb:exp_results_strategies}
        \vspace{-10pt}
    \end{minipage}
    
    \caption{The experiment results of meta-evaluation.}
    \label{tb:all_exp_results}
    \vspace{-20pt}
\end{table*}


==================================================
Filename: tables/exp_result_llmeval_detail.tex
==================================================
\begin{table*}[t]
    \small
    \centering
    ...
    \end{tabular}
    \end{adjustbox}
    \caption{The results of each human label in LLMEval$^2$. Only valid responses are counted when calculating the accuracy, while samples that couldn't receive responses due to triggering Azure OpenAI's content management policy are excluded. So there are some differences in the total values of different models.}
    \label{tb:exp_results_llmeval2_detail}
    \vspace{-20pt}
\end{table*}


==================================================
Filename: tex/6_Application.tex
==================================================
% \clearpage
% \begin{figure*}[t]
...
==================================================
Filename: tex/5_Experiments.tex
==================================================
...
==================================================
Filename: tex/1_Introduction.tex
==================================================
\clearpage
...
==================================================
Filename: tex/0_towards_AGI.tex
==================================================
% \section{Towards AGI: LLM-as-a-Judge as a Pillar}

% Human evolution is fundamentally driven by the ability to judge—to evaluate, adapt, and act in response to the environment. Judgment connects the biological, cognitive, and cultural aspects of evolution, enabling humans to shape their environments and themselves, as shown in Figure~\ref{fig:human_evolution}. Similarly, as AI systems like LLM-as-a-Judge are developed, they too rely on judgment to interact effectively with complex systems, solve problems, and drive their own evolution toward artificial general intelligence (AGI), illustrated in Figure~\ref{fig:teaser}.



% \subsection{Human evolution}
% At every stage of this evolutionary framework, judgment plays a foundational role. It is the mechanism that allows humans to process information, adapt to changing environments, and create new tools, behaviors, and cultural systems. Judgment is the key to solving complex problems, making decisions under uncertainty, and shaping both the natural and cultural environments in which humans exist.

% \paragraph{Natural Selection and Genes}
% The process of natural selection drives the evolution of genes, which lay the biological groundwork for the development of the human brain and nervous system. Genes determine the formation and functioning of neurons, the fundamental units of the brain, which enable higher cognitive processes such as reasoning, decision-making, and judgment. Judgment emerges as a critical capability at this stage. The ability to assess risks, evaluate opportunities, and make decisions based on incomplete information was essential for the survival and reproduction of early humans. This capability allowed humans to adapt to the natural environment by responding effectively to challenges such as finding food, avoiding predators, and choosing suitable habitats.

% \paragraph{Neurons and Cognition}
% Neurons, shaped by genetic and environmental influences, provide the neural basis for cognition, which encompasses reasoning, learning, memory, and problem-solving. Among these cognitive processes, judgment is fundamental. Judgment allows humans to process and evaluate information from their surroundings, identify patterns, and predict potential outcomes. For example, early humans used judgment to decide whether a certain food source was safe to eat, whether a shelter would protect them from harsh weather, or whether an approaching animal posed a threat. This capability to evaluate and act upon environmental stimuli is a direct result of the brain's ability to synthesize inputs and prioritize actions. Judgment, therefore, is not just a tool for survival but also the foundation for higher-order reasoning and strategic thinking.

% \paragraph{Cognition to Behavior}
% Judgment-based cognition translates into behavior, which represents the outward expression of human adaptation. Behavior shaped by judgment is what allowed humans to interact effectively with their environment. For example, hunting strategies, tool-making, social cooperation, and exploration were all behaviors driven by judgment. Judgment refines behavior by enabling humans to assess the success or failure of previous actions and adjust accordingly. This feedback loop between cognition and behavior enhances adaptability, allowing humans to thrive in diverse and changing environments.

% \paragraph{Behavior and Material Culture}
% Human behavior driven by judgment led to the creation of material culture, which includes tools, technologies, and artifacts that extend human capabilities. For example, the development of fire, weapons, and farming techniques were all results of humans applying judgment to solve problems and exploit opportunities in their environment. Material culture, in turn, transformed human interaction with the environment. By creating tools to hunt more effectively or technologies to cultivate crops, humans could control and modify their surroundings to better meet their needs. These innovations, driven by judgment, created a positive feedback loop, where the material environment further challenged and enhanced human cognitive and cultural evolution.

% \paragraph{Interaction with Natural and Cultural Environments}
% Humans are unique in their interaction with both the natural environment and the cultural environment, both of which heavily influence evolution. The natural environment presents challenges such as climate, resource availability, and predators, all of which require judgment to navigate and adapt to. The cultural environment, which consists of shared knowledge, practices, and societal norms, requires judgment to interpret social cues, resolve conflicts, and adapt to group dynamics. Judgment allows humans to evaluate and adapt to these environmental pressures effectively. 

% \paragraph{Feedback Loops in Evolution}
% The process of human evolution involves numerous feedback loops between its key components. The natural environment shapes genes through natural selection, while genes influence neurons and cognition, enabling judgment. Judgment, in turn, shapes behavior and material culture, which then alter both the natural and cultural environments. The cultural environment, through shared knowledge and societal structures, further accelerates the refinement of judgment and cognitive abilities. These feedback loops show how judgment acts as the central mechanism that connects biological, cognitive, behavioral, and cultural evolution. Without judgment, humans would not have been able to adapt as effectively, innovate as creatively, or cooperate as extensively.

% \subsection{Towards AGI by LLM-as-a-Judge}
% % The role of judgment in human evolution closely parallels the role of LLM-as-a-Judge in the development of artificial intelligence. Just as judgment refines human cognition and behavior, enabling humans to adapt and thrive, LLM-as-a-Judge refines the reasoning and problem-solving capabilities of large language models. By integrating judgment-based mechanisms, LLMs can achieve greater consistency, clarity, and adaptability in their interactions with complex environments, mirroring the evolutionary processes that have shaped human intelligence.


% % 1）类比论证是先决条件
% % 2）\到了现今的大模型时代，强大的基础模型已经展现出了强大的智能水平，能完成多种多样的任务。但它们也有个缺点，训练之后就基本定型了，无法自我进化。通过LLM-as-a-Judge可以跟人类进化一样跟environment交互，充当模拟器通往世界模型
% % 3）如果LLM-as-a-Judge可以Reliable的话，AI自我进化的路径被打通，As Figure 1所示。实际上generation和judge的evaluation实际上是一体两面。一个好的judge通常也是一个好的专家（无论是人还是LLM）。我们可以概念上想一想，当judge的任务次数goes to infinity，是不是对judge的要求就约等于generator了
% % 4）具体描述图中的内容以及o1等thinking模型
% % 5）总起后面的Application，Challenges，Futurework
% % \paragraph{Human Evolution: Judgment as a Prerequisite}


% % \paragraph{The Parallel with LLM-as-a-Judge}

% The role of judgment in human evolution closely parallels the role of LLM-as-a-Judge in the development of artificial intelligence. Just as judgment refines human cognition and behavior, enabling humans to adapt and thrive, LLM-as-a-Judge refines the reasoning and problem-solving capabilities of large language models (LLMs). By integrating judgment-based mechanisms, LLMs can achieve greater consistency, clarity, and adaptability in their interactions with complex environments, mirroring the evolutionary processes that have shaped human intelligence.
% % Figure Explanation and Thinking Models in Evolution
% Figure~\ref{fig:human_evolution} illustrates the interaction between judgment and environment in human evolution. As described, human cognition evolved through continuous feedback loops between neurons, cognition, behavior, and the cultural and natural environments. Similarly, in LLM development, a thinking model such as \(O_1\) exemplifies this iterative refinement process, where judgment is central to improving reasoning and aligning outputs with intended goals. LLM-as-a-Judge can be envisioned as the mechanism that bridges the gap between static models and dynamic, adaptive systems by simulating these feedback loops, allowing the model to refine its understanding and expand its capabilities over time.


% % \paragraph{Judgment as a Prerequisite for Evolution}
% % In both human evolution and AI development, the ability to judge is a foundational requirement for higher-order reasoning and adaptability. Human judgment, built through interaction with the environment, serves as a mechanism to evaluate options, weigh evidence, and refine decisions, thereby enabling survival and progress. Similarly, LLM-as-a-Judge provides large language models with the capability to assess, adapt, and improve their outputs, making it a prerequisite for achieving AGI. This analogy between human judgment and LLM-as-a-Judge underscores its critical role in developing systems that can evolve beyond their initial design.

% \paragraph{Overcoming Limitations of Static Models with LLM-as-a-Judge}
% In the era of large foundational models, current AI systems have demonstrated remarkable intelligence and versatility, capable of completing a wide variety of tasks. However, these models suffer from a major limitation: they are largely static after training and unable to self-evolve without external intervention. Unlike humans, who evolve by constantly interacting with their environment, existing LLMs lack the ability to refine themselves post-training. LLM-as-a-Judge addresses this gap by serving as a mechanism that enables models to interact with their environment, simulate complex scenarios, and function as a "world model simulator." Through iterative judgment and evaluation, LLMs can emulate the human-like process of adapting and evolving, creating a pathway toward self-improvement and autonomous learning.

% \paragraph{Reliable Judgment as the Pathway to AI Self-Evolution}
% If LLM-as-a-Judge can be made reliable, it paves the way for AI self-evolution, as illustrated in Figure~\ref{fig:teaser}. The key to this lies in the dual nature of generation and judgment: the process of generation (creating new outputs) and judgment (evaluating those outputs) are fundamentally two sides of the same coin. A good judge is often also a good expert, whether it is a human or an LLM. Conceptually, we can imagine that as the number of judgment tasks approaches infinity, the demands on a judge converge to those of a generator. In this way, reliable LLM-as-a-Judge mechanisms can act as the foundation for systems that continually refine and generate knowledge, creating a virtuous cycle of evaluation and self-improvement.

% The integration of judgment mechanisms into LLMs not only enables self-evolution but also opens up new avenues for applications across diverse fields, including decision-making, complex problem-solving, and ethical AI systems. However, significant challenges remain, such as ensuring the reliability of LLM-as-a-Judge systems, addressing biases, and maintaining alignment with human values. These challenges and potential applications will be discussed in subsequent sections, along with future directions for developing AGI through LLM-as-a-Judge.


==================================================
Filename: tex/8_Future_work.tex
==================================================
...
==================================================
Filename: tex/2_Formulation.tex
==================================================
...
==================================================
Filename: tex/3_Improvement/3_2_Improvement_Model.tex
==================================================
% \subsection[Improvement Strategy of LLMs' Abilities]{\textbf{Improvement Strategy of LLMs' Abilities}}\label{subsec:improve_models}
\subsection{Capability Enhancement Strategy}\label{subsec:improve_models}

The evaluation capabilities of LLMs are a reflection of their powerful general language understanding and generation abilities triggered by specific prompts. Methods for optimizing evaluation through prompt design——focused on LLMs' in-contextual learning capabilities——require LLMs to comprehend the meaning of prompts fully and consistently follow the relevant evaluation instructions. However, even state-of-the-art LLMs like GPT-4 encounter problems such as conceptual confusion\cite{hu-etal-2024-llm}, and smaller open-source LLMs have even more limitations in their evaluation capabilities.
% Consequently, enhancing the evaluation capabilities of LLMs, including how to fine-tune LLMs through meta-evaluation datasets and how to iteratively optimize models based on the feedback, is significant for improving the fundamental evaluation performance of LLM-as-a-judge.
Fine-tuning LLMs may be a common approach, but as mentioned at the end of Section~\ref{formulation:model_selection}, fine-tuned LLMs might exhibit limited generalization capability beyond their training data distribution, and could also remain susceptible to subtle biases within the fine-tuning data that compromise fairness, leading to inconsistencies with human judgment. Therefore, researchers propose two solutions: On one hand, constructing fairer training datasets to fine-tune LLMs and eliminate judgment biases; on the other hand, implementing feedback-driven iterative refinement methods, which continuously update LLMs during usage to enhance their generalization capability, ultimately achieving reliable evaluation models.

% \subsubsection{\textbf{Fine-tuning via Meta Evaluation Datasets}}\label{subsubsec:finetune_model}
\subsubsection{\textbf{Specialized Fine-tuning}}
\label{subsubsec:finetune_model}

\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/chap3_process_sft_data_construction.png}
    \caption{Two paradigms of the construction process of meta evaluation datasets for training.}
    \label{fig:process_sft_data_construction}
\end{figure}
...
% Fig~\ref{fig:illustration_two_paradigms}II
As shown in Fig~\ref{fig:process_sft_data_construction}, there are primarily two approaches to constructing meta-evaluation data: \textit{Evaluation Templates} and \textit{Deep Transformation}. The former typically populates sampled raw data into preset templates to form training data, while the latter employs algorithms or models to transform raw data in terms of style, content, and structure, thereby constructing training data more flexibly.

Using evaluation templates is a common method, which involves sampling evaluation questions from publicly available datasets, modifying them with certain templates, and supplementing the dataset with evaluation responses generated either manually or by powerful LLMs like GPT-4. For instance, PandaLM\cite{wang2023pandalm} samples inputs and instructions from Alpaca 52K\cite{alpaca} and generates responses using GPT-3.5 to construct training data, while SALAD-Bench\cite{salad_bench} builds its training data from a subset of LMSYS-Chat\cite{zheng2023lmsys} and Toxicchat\cite{lin2023toxicchat}. 

To better align with the requirements of evaluation tasks, many research works further transform inputs and instructions sampled from public datasets to construct more targeted training data. OffsetBias\cite{offsetbias} aims to reduce biases of LLMs by using GPT4 to generate off-topic versions of the original inputs and then having GPT-3.5 respond to the new inputs to produce bad responses. By pairing good and bad responses as training data to fine-tune the LLMs as evaluators, the biases in LLMs are significantly reduced, including length bias, concreteness bias, knowledge bias, and so on. JudgeLM\cite{zhu2023judgelm} enhances LLMs' evaluation capabilities by creating different types of training data through paradigms like reference support and reference drop. CritiqueLLM\cite{ke2024critiquellm} proposes a multi-path prompting approach, combining pointwise-to-pairwise and referenced-to-reference-free prompting strategies to restructure referenced pointwise grading data into four types, which helps create Eval-Instruct to fine-tune LLMs, addressing shortcomings in pointwise grading and pairwise comparison. Yu et al.\cite{yu2025improve} sample data from the preference dataset and rewrite judge templates to synthesize training data, and use GPT-4o to analyze and judge the pairs of answers in the synthetic data. The correct judgments, along with the synthetic data and analysis, will be processed as training data for the SFT phase of judge models.
...
% \subsubsection{\textbf{Iterative Optimization Based on Feedback of Evaluation Results}}\label{subsubsec:iterate_models}
\subsubsection{\textbf{Feedback-Driven Iterative Refinement}}
\label{subsubsec:iterate_models}
...
A typical example is INSTRUCTSCORE\cite{xu2023instructscore}. To improve model performance and further benefit the final quality score calculation, this scoring framework collects failure modes of metric outputs, queries GPT-4 on each failure mode to gather automatic feedback, and finally selects explanations most aligned with human preferences to iteratively fine-tune the LLaMA model. Unlike INSTRUCTSCORE, which directly optimizes the model, the LLM evaluator in JADE\cite{zhang2023jade} relies on human judges to correct LLMs' evaluation results and updates the most frequently corrected samples into the example sets for few-shot prompting. JADE utilizes this relatively low-cost method to achieve iterative updates of the evaluation capabilities. 

Iterative optimization methods are not limited to offline training. The success of R1-style methods shows the effectiveness of online reinforcement learning (RL) approaches. Thus, feedback-based online learning constitutes another strategic form of iterative optimization. Think-J\cite{huang2025think} integrates both offline and online learning approaches. The offline approach trains a critic model to evaluate judgments from the judge model, thereby constructing positive and negative samples for SFT and DPO optimization. The online approach continuously optimizes the judge model using the Group Relative Policy Optimization (GRPO) algorithm with rule-based rewards as optimization feedback.
...
==================================================
Filename: tex/3_Improvement/3_1_Design_Prompts.tex
==================================================
% \subsection[Design Strategy of Evaluation Prompts]{\textbf{Design Strategy of Evaluation Prompts}}\label{subsec:design_prompts}
\subsection{Prompt Design Strategy}\label{subsec:design_prompts}
% \jw{jw: discussion on better design of prompt involving evaluation forms, such as 1-5 vs 1-10, Relative evaluation and absolute evaluation}

An evaluation prompt is an input to LLM evaluators, which is used to guide the LLMs to complete the required evaluation tasks. LLMs possess in-context learning ability, enabling them to learn how to perform specified tasks from relevant examples or instructions in prompts,  without requiring weight updates or retraining\cite{brown2020language}. This suggests that the design strategy of evaluation prompts will significantly impact the effectiveness of LLM-as-a-judge. 
% Therefore, optimizing the design of evaluation prompts, including better methods to help LLMs interpret the evaluation tasks and generate results, is the most direct and effective way to boost the evaluation performance of LLM-as-a-judge.
Therefore, to achieve reliable evaluations, researchers have experimented with diverse prompt design strategies, including improving LLMs' task understanding and standardizing LLMs' output format in the prompt. This approach mitigates reliability concerns of In-context learning mentioned in Section~\ref{formulation:icl}, such as unstable results, inter-rater inconsistency, ambiguity in response, and positional or length biases.


% \subsubsection[Optimizing LLMs' Understanding of Evaluation Tasks]{\textbf{Optimizing LLMs' Understanding of Evaluation Tasks}}\label{subsubsec:design_input}
\subsubsection{\textbf{Improving LLMs' Task Understanding}}
\label{subsubsec:design_input}

In optimization methods of prompting LLMs to better understand evaluation tasks, one of the most commonly used and effective approaches is few-shot prompting\cite{brown2020language}. By incorporating several high-quality evaluation examples into the evaluation prompts, LLM evaluators can effectively grasp the objectives, general processes, and rough evaluation criteria of evaluation tasks. Many research works employ this prompt paradigm for evaluation, such as FActScore\cite{min2023factscore}, SALAD-Bench\cite{salad_bench}, and GPTScore\cite{fu2024gptscore}.
...
\begin{itemize}
    \item \textbf{(a) Decomposition of Evaluation Steps} entails breaking down the entire evaluation tasks into smaller steps, providing detailed definitions and constraints for each small step in prompts, thereby guiding LLMs comprehensively through the whole evaluation pipeline. For instance, G-Eval\cite{geval} and DHP\cite{dhp} use Chain-of-Thought(CoT)\cite{cot} to guide LLMs. SocREval\cite{he2024socreval} employs the Socratic method to meticulously design each step to enhance evaluation performance. In existing research, this paradigm resembles providing detailed and sequential execution steps within a single prompt for a one-time evaluation, which guides the LLM-as-a-judge to complete evaluation tasks aligned with requirements, as illustrated in Fig~\ref{fig:pipeline_decomposition}(a).
    % Fig~\ref{fig:illustration_two_paradigms}I.(a).
    To better understand this paradigm, we present a demonstrative example prompt for evaluating summary quality: \textit{"You will be given a summary written for an article. ……  Evaluation Steps: 1. Read the Summary Thoroughly: Before diving into the evaluation, ensure that you ...; 2. Identify the Central Topic: A coherent summary will have a clear central topic or theme ...; 3. Look for Transitional Elements: Coherent summaries often have clear transitions between sentences or ideas ...; 4. Check for Logical Flow: Review the summary for logical sequencing ... X. Give a Score: ... Source Article: \{\{ Article \}\} Summary: \{\{ Summary \}\}".} In this example, the evaluation task for summary quality is decomposed into many small steps to guide the LLM through the entire assessment process—from reading and analyzing to scoring—enabling methodical evaluation of a summary. 
    
    \item \textbf{(b) Decomposition of Evaluation Criteria} involves breaking down coarse evaluation criteria like Fluency into finer-grained sub-criteria like Grammar, Engagingness, and Readability, and then generating overall scores based on these different dimensions. Saha et al. propose Branch-Solve-Merge(BSM)\cite{bsm}, which divides evaluation tasks into multiple parallel sub-tasks based on different sub-criteria for separate evaluation and final merge. HD-Eval\cite{hd-eval} proposes a hierarchical criteria decomposition method to iteratively align LLM evaluators with human preference, thereby addressing the potential bias in LLMs. Hu and Gao et al.\cite{hu-etal-2024-llm} summarize and clearly define an explicit hierarchical classification system encompassing 11 criteria, addressing the issue of LLMs potentially confusing different evaluation standards. Different from the decomposition of steps, this paradigm presents greater complexity due to significant variations in evaluation requirements and procedures across different criteria dimensions. Current methodologies tend to decompose the original evaluation task into multiple sub-tasks based on distinct criteria. Each sub-task is assessed via a distinct prompt, with results subsequently merged or simply listed in a score table, as demonstrated in Fig~\ref{fig:pipeline_decomposition}(b).
    % Fig~\ref{fig:illustration_two_paradigms}I.(b).
\end{itemize} 

% \begin{figure}
%     \centering
%     \includegraphics[width=0.9\linewidth]{figures/chap3_Illustration_of_two_paradigms.png}
%     \caption{The evaluation pipeline of two decomposition paradigms (I.(a) and I.(b) in the upper half of the image), and two paradigms of the construction process of meta evaluation datasets for training (II.(a) and II.(b) in the lower half).}
%     \label{fig:illustration_two_paradigms}
% \end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/chap3_pipeline_decomposition.png}
    \caption{Simplified Evaluation Pipeline of Two Decomposition Paradigms.}
    \label{fig:pipeline_decomposition}
\end{figure}
...
Wang et al.\cite{wang-etal-2024-large-language-models-fair} analyzed and validated the impact of position bias on LLM-as-a-judge and proposed a calibration framework to mitigate this bias by swapping the contents and averaging the scores. Auto-J\cite{auto-j} and JudgeLM\cite{zhu2023judgelm} also enhance the evaluation consistency by shuffling the texts to be evaluated. In contrast to averaging scores, PandaLM\cite{wang2023pandalm} annotates the conflicting evaluation results after swapping as "Tie" to address the position bias.
...
To address the challenge of LLMs' absolute scoring being less robust than relative comparing\cite{raina2024llm}, some research works convert scoring tasks into pairwise comparison, thereby enhancing the reliability of evaluation results. 
Liu et al.\cite{liu2024aligning} transform the scoring evaluation to ranking evaluation and introduce Pairwise-Preference Search (PARIS), which employs LLMs to conduct pairwise comparisons locally and efficiently ranks candidate texts globally, making evaluation results more aligned with human preferences.
...
By refining task instructions and criteria in prompts or few-shot prompting with high-quality examples, the details of evaluation prompts can be enriched, and the understanding of LLMs on evaluation tasks can be directly or indirectly enhanced. Additionally, targeted adjustments to prompts can address potential biases of LLMs, such as position bias. Thus, by improving LLMs' task understanding, the inconsistent inter-rater reliability, the ambiguity in response, and the biases mentioned in Section~\ref{formulation:icl} can be solved. 




\subsubsection{\textbf{Standardizing LLMs' Output Format}}
\label{subsubsec:design_output}
...
An effective method to enhance the robustness of output forms is to constrain the output in structured formats within prompts. G-Eval\cite{geval} and DHP framework\cite{dhp} perform evaluation tasks with a form-filling paradigm, constraining outputs with formats like \textit{"X: Y"}, where \textit{X} represents the dimension or metric to be evaluated and \textit{Y} denotes an identifiable output form like scores or specific tokens. LLM-EVAL\cite{llmeval} further modifies this form-filling paradigm, efficiently outputs evaluation results in JSON format, and obtains multidimensional scores, leveraging LLMs' high understanding and generation capabilities of code-like textural formats.

Apart from challenges in robustness, directly outputting evaluation results by LLMs also suffers from a lack of interpretability. The meaning of evaluation results from LLM evaluators is difficult to align consistently with the instructions and metrics provided in prompts. To address the challenges, CLAIR\cite{chan2023clair} requires LLMs to output evaluation scores between 0-100 simultaneously with relevant reasons as explanations in JSON format, which enhances the rationality and interpretability of the scores. FLEUR\cite{lee-etal-2024-fleur} utilizes LLaVA to first provide quality scores for image captions and subsequently asks with \textit{"Why? Tell me the reason."} for explanations with the images, captions, and scores as inputs, offering a stepwise approach to provide interpretable scores. 
...
==================================================
Filename: tex/3_Improvement/3_3_Optimization_Results.tex
==================================================
% \subsection[Optimization Strategy of Final Results]{\textbf{Optimization Strategy of Final Results}}\label{subsec:optimization_results}
\subsection{Final Output Optimization Strategy}\label{subsec:optimization_results}
% \jw{jw: discussion on the number of selected evaluation results can be added. For example, the difference between odd and even numbers and the difference in quantity.}
...
% However, the inherent generation randomness within the black box of LLMs still introduces significant instability to the entire evaluation pipeline, affecting the overall evaluation quality. 
...
manipulations, as noted at the end of Section~\ref{formulation:post_processing}, may collectively contribute to unreliable and unfair evaluation results.
...
% In this survey, these optimization strategies are categorized into three types: integration of multiple evaluation results, direct optimization of LLMs' outputs, and conversion of evaluation tasks from pointwise evaluation to pairwise comparison.
...
% \subsubsection{\textbf{Integration of Multiple Evaluation Results}}\label{subsubsec:integrate_multiple}
\subsubsection{\textbf{Integrating Multi-Source Evaluation Results}}\label{subsubsec:integrate_multiple}
...
The most basic optimization strategy is to perform multiple runs of evaluation on the same content with different hyper-parameters and settings, and then summarize these results. For example, the work of Sottana et al.\cite{sottana2023evaluation} reduces randomness in evaluations by averaging multiple scores of the same sample. Similarly, PsychoBench\cite{psycho_bench} takes the mean and standard deviation from ten independent runs. Auto-J\cite{auto-j} further amplifies the differences between evaluation rounds, which combine critiques with and without scenario criteria to obtain the final results.

In addition to integrating results from multiple rounds of evaluation, using multiple LLM evaluators to assess the contents simultaneously and integrating the results is another effective method, which can reduce biases introduced by LLMs. For instance, CPAD\cite{cpad} utilizes ChatGLM-6B\cite{chatglm}, Ziya-13B\cite{ziya}, and ChatYuan-Large-v2\cite{chatyuan} as evaluators to evaluate the contents and obtain the final results by voting. Bai et al.\cite{lm-as-an-examiner} propose a novel evaluation method called decentralized peer review of LLMs, which utilizes LLMs that generate content to evaluate each other's generated content and eventually integrate the results. 
EvalMORAAL\cite{mohammadi2025evalmoraal} also employs a similar LLM-as-a-judge peer review mechanism, detecting conflicts in evaluation results by checking whether score differences exceed a preset threshold, and uses majority voting to resolve conflicts.

The aforementioned multi-round or multi-model approaches represent a type of peer-parallel evaluation strategy, and their integration process is relatively straightforward. Researchers have also proposed many more complex evaluation frameworks featuring greater interactivity between different evaluations. For example, Jaehun Jung et al. proposed Cascaded Selective Evaluation\cite{jung2024trust}. This framework transitions from weaker, smaller models to stronger, larger models based on confidence, allowing the majority of evaluations to be handled by smaller models, which significantly reduces the costs of computing resources and improves efficiency. Recent work \cite{zhang2025crowd}, on the other hand, proposed Crowd-based Comparative Evaluation. This method utilizes multiple LLMs to construct crowd responses based on candidate responses for comparison, thereby generating multiple crowd judgments that serve as reference content for the final evaluation. This enables LLM-as-a-Judge to capture richer details and enhance evaluation reliability.
...
% \subsubsection{\textbf{Direct Optimization of LLMs' Outputs}}\label{subsubsec:process_outputs}
\subsubsection{\textbf{Direct Output Optimization}}
\label{subsubsec:process_outputs}
...
For example, FLEUR\cite{lee-etal-2024-fleur} proposes a score smoothing strategy. For scores generated by LLaVA, the probability of the token corresponding to each digit $l$ (0$\leq l\leq$9) would be used as the weight to smooth the explicit scores and calculate the final evaluation scores.
TrustJudge\cite{wang2025trustjudge} proposes a distribution-sensitive scoring method that computes continuous expectations from discrete scoring probabilities, and a likelihood-aware aggregation method that utilizes bidirectional preference probabilities, to process the evaluation results from LLMs, addressing the inconsistency in LLM-as-a-Judge.

However, methods like score smoothing, which combine implicit logits and explicit outputs, require the LLMs to be open-source or to provide interfaces that allow access to token probabilities, which brings some limitations. Inspired by the work of Weng et al.\cite{weng-etal-2023-large} and Madaan et al.\cite{madaan2024self}, self-verification can be used to filter out the evaluation results without sufficient robustness. For example, TrueTeacher\cite{gekhman2023trueteacher} applies self-verification in its evaluation of distilled data by asking the LLM evaluator for its certainty about the evaluation results after providing them and retaining only those results that pass self-verification. Self-verification is suitable for all LLMs and requires no complex computing and processing.
...
==================================================
Filename: tex/3_Improvement/3_Improvement.tex
==================================================
...
==================================================
Filename: tex/Practical_Guide.tex
==================================================
...
% As shown in Figure~\ref{fig:quick_practice}, \textbf{The process of quick practice for LLM-as-a-Judge involves four main stages.} First is the thinking phase,  in which users define the evaluation objectives by determining what needs to be evaluated, understanding typical human evaluation approaches, and identifying some reliable evaluation examples. Next is prompt design, detailed in Section~\ref{formulation:icl}, both wording and formats matter.
...
% start by crafting the initial prompt that guides the model's understanding of the task. Next, select an appropriate model based on its ability to process and respond within the given context. In addition, choosing a general model or fine-tuning a model for the evaluation task, as well as selecting the appropriate model size, also involves cost considerations.
% Following model selection, implement post-processing methods to refine the model's outputs, enhancing clarity and relevance. Finally, evaluate the outcomes to ensure the model's performance aligns with the desired objectives. This sequence from in-context learning to evaluation forms a cohesive approach to leveraging language models for specific tasks.
