...
# CoT3.static → CoT3.recheck → CoT3.judge까지 명령어 한 방으로 잇는다. (CoT5/재탐색 루프는 아직 미배선.)
#
...
def get_args():
    ...
    parser.add_argument("--paper_ids", type=str, nargs="+",
                        help="--papers와 같은 순서의 arXiv ID")
...
def build_canaries(pool):
    ...
    # (구 음성 CANARY_NOV[novelty]는 제거됨 — novelty는 CoT3 판정자에서 빠지고 CoT5로 이관되어 여기서 검증 대상 아님.)
...
async def main():
    args = get_args()
    if args.paper_ids and len(args.paper_ids) != len(args.papers):
        raise ValueError("--paper_ids must have the same number of values as --papers")
    paper_ids = dict(zip(args.papers, args.paper_ids or []))
