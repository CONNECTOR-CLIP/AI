...
from noveltychecker.models.idea_novelty_checker.paper_collection import (
...
async def get_review_helper(evaluation_papers):
    ...
    category, review, output_text = await get_review(idea, evaluation_papers)
...
async def run_ideanoveltychecker(
    ...
    cutoff_date: CoT5 patch (README 4.2절 "추가 설계"). "YYYY-MM-DD" 문자열. RankGPT 재정렬까지 끝난
        `most_relevant_papers`에서 cutoff_date 이후 출판된 후보를 top-k로 자르기 *전에* 제거해,
        시간 누출된 문헌이 최종 리뷰 근거로 쓰이지 않게 한다.
    """
    ...
    dropped_by_cutoff = []
    if cutoff_date and len(retrieval_trace.get("most_relevant_papers", [])) != 0:
        from cot5.cutoff import filter_by_date  # CoT5 patch

        records = retrieval_trace["most_relevant_papers"].to_dict(orient="records")
        kept_records, dropped_by_cutoff = filter_by_date(records, cutoff_date)
        retrieval_trace["most_relevant_papers"] = pd.DataFrame(kept_records)
    retrieval_trace["dropped_by_cutoff"] = dropped_by_cutoff
