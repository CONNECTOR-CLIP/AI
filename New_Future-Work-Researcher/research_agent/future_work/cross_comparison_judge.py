...
#   백오프 재시도. 폐기: RUBRIC(1~5)·COMPARATIVE_CALIBRATION·cause_tag·novelty(→CoT5).
...
SEPARATE later stage (CoT5) checks — they are NOT grounding failures here, so do NOT add them as
...
not by the words:
  ...
  - SKIP it (do NOT add it, do NOT label it "neutral") if X is a capability belonging to ANOTHER paper
    ...
    NOVELTY setup, which CoT5 checks. Example: "P1 does not address parameter-efficient adaptation"
...
# 새로운가"는 CoT5가 판단할 몫이기 때문. (주의: 이건 제거된 'novelty 점수 축'이 아니라, gap 문장 속 부재
...
def _is_absence_claim(text: str) -> bool:
    """조각 텍스트가 '세상-전체 부재 주장'인지(= grounding에서 제외 대상, CoT5 몫). 대소문자·공백 정규화 후 부분일치."""
...
def _parse_verdict(raw: Optional[str]) -> Optional[dict]:
    ...
    # [코드 안전망] LLM이 스킵 못 한 '세상-전체 부재' 조각을 grounding 집합에서 제외(CoT5 몫) — CRITERIA와 이중 방어.
...
def judge_candidates(
    ...
    Returns:
        (kept_candidates, log)  — kept는 CoT5로 넘어갈 대상(FAIL만 drop), log는 KEEP/DROP/사유
    """
